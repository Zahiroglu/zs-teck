...
class ControllerRegisterUserself extends GetxController{
  ...
  RxList<ModelRegion> listRegionlar =List<ModelRegion>.empty(growable: true).obs;
  ...
  Future<void> getCompanyDetails(String lisanceId, String token, String companyId, String moduleId, String roleId,) async {
    dataLoading.value=true;
    update();
    await FirebaseFirestore.instance.collection('db_companies').where('companyId', isEqualTo: companyId).get().then((QuerySnapshot querySnapshot) async {
      if (querySnapshot.docs.isEmpty) {
        // bu halda xeta bas verir
      } else {
        await FirebaseFirestore.instance.collection('db_companies').doc(querySnapshot.docs.first["companyId"]).collection("regions").get().then((querySnapshot2) {
              for(var reg in querySnapshot2.docs){
                ModelRegion modela = ModelRegion(
                    ...
                    regionLatitude: double.tryParse(reg["regionLatitude"].toString()),
                    regionLongitude: double.tryParse(reg["regionLongitude"].toString()));
  ...
  Future<void> getModelInfo(String lisanceId, String token, String companyId, String moduleId, String roleId,) async {
    String languageIndex = await getLanguageIndex();
    print("languageIndex : "+languageIndex.toString());
    print("moduleId : "+moduleId.toString());
    update();
    await FirebaseFirestore.instance.collection('db_modules').doc(languageIndex).collection("modules").where("moduleId",isEqualTo:moduleId )
        .get().then((QuerySnapshot querySnapshot) async {
      if (querySnapshot.docs.isEmpty) {
        // bu halda xeta bas verir
      } else {
        moduleName.value=querySnapshot.docs.first['moduleName'];
        await FirebaseFirestore.instance.collection('db_roles').doc(languageIndex).collection("roles").where("roleId",isEqualTo:roleId )
            .get().then((QuerySnapshot querySnapshot) async {
          if (querySnapshot.docs.isEmpty) {
            // bu halda xeta bas verir
          } else {
